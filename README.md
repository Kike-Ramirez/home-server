...
| `restic-check.sh` | Verificación de integridad (cron día 15, 02:00): repo local con `--read-data-subset=10%` (rota sobre todo el repo en ~10 meses), repo offsite solo estructura/índices (sin descargar packs, para no gastar cuota de la API de Drive). Métrica + alerta en Grafana si algún repo lleva >40 días sin un check exitoso. |
| `logrotate.conf` | Instalado en `/etc/logrotate.d/homelab` (no vía cron propio). Rota `backups/*.log` + `telegram-agent/agent.log`: semanal, 8 semanas de histórico, comprimido. |
